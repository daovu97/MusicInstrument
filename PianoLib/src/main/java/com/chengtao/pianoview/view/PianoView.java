...
public class PianoView extends View {
    ...
    public void initPiano() {
        ...
        piano = new Piano(context, scale, isCorD0);
        //获取白键
...
//            whitePianoKeys.addAll(piano.getWhitePianoKeys());
            for (int i = 0; i < whitePianoKeys.size(); i++) {
                for (PianoKey key : getWhitePianoKeys().get(i)) {
        ...
        for (int i = 0; i < whitePianoKeys.size(); i++) {
            for (PianoKey key : whitePianoKeys.get(i)) {
                if (!key.isPressed() && key.contains(x, y)) {
                    ...
                    Log.e("chiennc", "handleDown: isHere");
                }
            }
        }
        //检查黑键
...
//        invalidate();
     ...
     */
    public void autoPlay(final List<AutoPlayEntity> autoPlayEntities, ProgressBar progressBar, int progressSpeed, ProgressBar progressSpeedBar) {
        if (isAutoPlaying) {
            return;
        }
        isAutoPlaying = true;
    ...
    public void autoPlay2(final List<AutoPlayEntity> autoPlayEntities, ArrayList<PianoKey[]> whitePianoKeys) {
        if (isAutoPlaying) {
            return;
        }
        isAutoPlaying = true;
